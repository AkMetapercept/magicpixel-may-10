...
## Usage
The **Usage** option includes all the real-time metrics and the data that are collected based on the number of days. By default, the data for the last seven days appears. 
...
| ----------- | ----------- |
| Tag Manager    | In Tag Manager, there are a set of API calls triggered to MagicPixel from the client website through SDK. The Tag Manager in the Usage option includes the metrics that are collected in real-time in making the server-side API calls. The Tag Manager includes the following products: <br /> • Tag Manager <br /> • Id link |
...
| Collector Bot Traffic | Number of API calls detected as bot traffic by MagicPixel servers.|

The same categories are listed under the **All Categories** drop-down to view more detailed view of the metrics. For example, to view the Collector API calls for the Last One Week, click Collector API Calls from the **All Categories** drop-down and select Last One week. To view the graphical representation for the last one month, select Last One Month and click **Refresh**.
 
**Id Link**
The ID Link as a product does not include Projects drop-down option. You can select the particular category using the **All Categories** drop-down. The only category that is available under All Categories is **First Party ID Service**.

## Notifications

The **Notifications** option displays the notifications that are triggered for a particular user. You can add any number of e-mail IDs for a particular Notification Group. The Notifications are triggered at the client level for alarms as well across multiple products.
**Note**: While creation of the alarms, you can specify the notifications group to send the e-mails whenever an alarm is triggered. The two types of alarms that are triggered under Tag Manager are: <br /> •	 Tag Expiration Alarm <br /> •	 Anamoly Detection Alarm
